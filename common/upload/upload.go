...
func New(options ...UploadOption) *Upload {
	...
	config := &Config{
		...
		UrlPath:    config.Data.Lili.UploadPath + todayPath,
		SavePath:   config.Data.Lili.UploadPath + todayPath,
		RandomName: false,
	}
...
func (u *Upload) Save() (*Result, error) {
	...
	if config.Data.Lili.Domain == "" {
		return nil, errors.New("未找到域名，无法生成访问地址")
	}

	if config.Data.Lili.SSL {
		domain = "https://" + config.Data.Lili.Domain
	} else {
		domain = "http://" + config.Data.Lili.Domain
	}
